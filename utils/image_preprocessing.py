"""Image padding used to bring M4D SAR images to a fixed network input size.

The M4D images are padded to a common size using a fixed border taken from an
anchor image (following the HTSM training template referenced in the paper).
"""
...
class ImagePadder:
    ...
    def _set_anchor_image_resized(self):
        height, width = self._anchor_image.shape[0], self._anchor_image.shape[1]
